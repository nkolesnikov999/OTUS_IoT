...
PORT = 1883
ClientID = "mqtt-nkolesnikov999-glym6p"
...
### machine and logic
class Machine():
    def __init__(self, mqtt_client, autocontrol=True, indexCoordinates=0, longtitude=coordinates[0][0],
                 latitude=coordinates[0][1], charge=120, speed=0, state=False):
        self.mqtt_client = mqtt_client
        self.topic_data = "data/pos"
    ...
    def command_change_state(self, client, userdata, message):
        ...
        try:
            self.state = int(message.payload.decode())
        except:
    ...
    def command_change_control(self, client, userdata, message):
        ...
        try:
            self.autocontrol = int(message.payload.decode())
        except:
    ...
    def get_data(self):
        data = json.dumps(
            {"state": self.state, "autocontrol": self.autocontrol, "charge": self.charge, "speed": self.speed, "lon": self.longtitude,
             "lat": self.latitude, "time": str(datetime.now())})
        return data
...
def on_connect(client, userdata, flags, rc):
    print("\nConnected with result code %s " % str(rc))
    if rc == 0:
        isConnect = 1
...
def run(client, host=HOST, port=PORT):
...
def main():
    ...
    mqtt_client = init(ClientID, ClientUsername, ClientPassword)
    run(mqtt_client)
    # read config
    autocontrol = True
    indexCoordinates = 0
